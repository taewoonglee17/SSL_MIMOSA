...
import torch
from fastmri.data import CombinedSliceDatasetQALAS, SliceDatasetQALAS
...
def worker_init_fn(worker_id):
    ...
    data: Union[
        SliceDatasetQALAS, CombinedSliceDatasetQALAS
    ...
    if isinstance(data, CombinedSliceDatasetQALAS):
        for i, dataset in enumerate(data.datasets):
            if dataset.transform.mask_func_acq1 is not None:
                if (
                    is_ddp
                ...
                dataset.transform.mask_func_acq1.rng.seed(seed_i % (2 ** 32 - 1))
                dataset.transform.mask_func_acq2.rng.seed(seed_i % (2 ** 32 - 1))
        ...
        if is_ddp:  # DDP training: unique seed is determined by worker and device
            ...
            seed = base_seed
        data.transform.mask_func_acq1.rng.seed(seed % (2 ** 32 - 1))
        data.transform.mask_func_acq2.rng.seed(seed % (2 ** 32 - 1))
...
class FastMriDataModuleQALAS(pl.LightningDataModule):
    ...
    def _create_data_loader(
        ...
        dataset: Union[SliceDatasetQALAS, CombinedSliceDatasetQALAS]
        if is_train and self.combine_train_val:
            ...
            dataset = CombinedSliceDatasetQALAS(
                roots=data_paths,
