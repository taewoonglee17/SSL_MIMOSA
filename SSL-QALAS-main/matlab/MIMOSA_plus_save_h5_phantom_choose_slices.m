...
% Choose specific slices
slc_idx = [17 18 19 28 29 30 37 38 39];
input_img = input_img(:,:,slc_idx,:,:);     % [Nx,Ny,3,1,Nacq]
T1_map    = T1_map(:,:,slc_idx);            % [Nx,Ny,3]
T2_map    = T2_map(:,:,slc_idx);
T2s_map   = T2s_map(:,:,slc_idx);
PD_map    = PD_map(:,:,slc_idx);
IE_map    = IE_map(:,:,slc_idx);
B1_map    = B1_map(:,:,slc_idx);
Nz        = numel(slc_idx);                 % update Nz
%% Brain masks
